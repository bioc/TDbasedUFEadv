...
```
Here we tries to find if Cisplatin causes distince expression between two 
...
```
Next we select which genes' epxression is alterd by Cisplatin.
```{r}
index <- selectFeature(HOSVD,input_all,de=0.05)
```
List the genes selected.
```{r}
