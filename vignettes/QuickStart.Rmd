...
```
Although we do not repeat the process select the eighth one as shown below
![The eighth singular value vector](./fig9.jpg)

![Left: Dependence of standard deviation of histogram of P-values. Right: Histogram of 1-P, for ditinct expression between cell lines treated with Cisplatin and other cell lines, i.e. for expDrug.](./fig10.jpg)

![Left: Dependence of standard deviation of histogram of P-values. Right: Histogram of 1-P, for ditinct expression between cell lines treated with Cisplatin and other cell lines, i.e. for expDrug.](./fig11.jpg)

Since package does not allow interactive mode, we place here batch 
mode as follows.
```{r}
index_all <- selectFeatureTransRect(HOSVD,cond,de=c(0.01,0.01),input_all=8)
```
```{r}
