...
```{r}
Z <- prepareTensorfromMatrix(t(CLL_data$Drugs[seq_len(200),seq_len(100)]),
t(CLL_data$Methylation[seq_len(200),seq_len(100)]))
sample<- outer(rownames(CLL_data$Drugs)[seq_len(200)],
rownames(CLL_data$Methylation)[seq_len(200)],function(x,y){paste(x,y)})
require(TDbasedUFE)
Z <- PrepareSummarizedExperimentTensor(sample=sample,
feature=colnames(CLL_data$Drugs)[seq_len(100)],value=Z)
```
