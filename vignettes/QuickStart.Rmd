...
Since this one does not reprsetn distinction we continue to type 1 and pres 
...
HOSVD to a tensor. In this case, we apply SVD instead of HOSVD to matrix 
generated from a tensor as follows.
In contrast to the above where only top two hudred features and top one hundred 
samples are included, the following one include all features and samples since
it can save require memory because partial summation of features.
``` {r}
library(Biobase)
...
identify features associated e with altered expression by treatment of
